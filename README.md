# STM32F407 环境监测与 MQTT 下位机

本工程是一个基于 STM32F407ZG 的物联网下位机程序，使用 ThreadX 进行多线程调度，通过 DHT11 采集温湿度，OLED 显示本地状态，并通过 ESP8266/ESP-AT 接入 Wi-Fi 与阿里云 MQTT。设备支持网页配网、Flash 保存 Wi-Fi 参数、周期上报、云端命令控制、定时开关、阈值自动控制和心跳/事件/回复消息。
...
| 线程 | 优先级 | 栈大小 | 职责 |
| --- | --- | --- | --- |
| `led_thread` | 1 | 1024 B | `PF9/PF10` 交替闪烁，表示系统运行 |
...
- 无法收到下行命令时，确认云端发布到 `/k0xzrztwuSU/zheng/user/cmd`，并使用合法 JSON。
